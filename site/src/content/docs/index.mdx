---
title: Arcnem Vision
description: Teach machines to see. Let agents decide what to do about it.
template: splash
hero:
  tagline: Arcnem Vision is an image-ingestion and analysis platform built around a server-side workflow engine. Images arrive through device API keys or the dashboard, workflows are loaded from database state, MCP tools perform the analysis, and the dashboard exposes the resulting documents, artifacts, and run traces.
  image:
    file: ../../assets/logo.svg
  actions:
    - text: Get Started
      link: /getting-started/
      icon: right-arrow
      variant: primary
    - text: View on GitHub
      link: https://github.com/arcnem-ai/arcnem-vision
      icon: external
---

import { Card, CardGrid } from '@astrojs/starlight/components';

## Features

<CardGrid stagger>
  <Card title="Two ingestion paths" icon="random">
    Support automated device/API-key uploads and operator-driven dashboard uploads on the same platform.
  </Card>
  <Card title="Database-driven workflows" icon="setting">
    Define workflows as data, not code. Mix worker, tool, supervisor, and condition nodes without redeploying.
  </Card>
  <Card title="MCP-backed analysis" icon="open-book">
    OCR, descriptions, embeddings, segmentation, similarity search, and retrieval are exposed as MCP tools for workflows and operators.
  </Card>
  <Card title="Persistent analysis corpus" icon="document">
    Store source images plus descriptions, OCR, embeddings, segmentations, and derived images for later search and reuse.
  </Card>
  <Card title="Dashboard control plane" icon="pencil">
    Manage projects, devices, API keys, workflows, uploads, search, chat, and runs from one operating surface.
  </Card>
  <Card title="Run trace visibility" icon="bars">
    Inspect initial state, per-step deltas, final state, timing, and errors for every workflow execution.
  </Card>
  <Card title="Grounded retrieval and chat" icon="comment-alt">
    Search by meaning and ask questions against the persisted corpus with source-backed answers.
  </Card>
</CardGrid>

## Tech Stack

| Layer | Tech | What it does |
|---|---|---|
| **API** | Bun, Hono, better-auth, Inngest, Pino | Presigned uploads, auth, workflow triggers, structured logging |
| **Dashboard** | React 19, TanStack Router, Tailwind, shadcn/ui | Workflow builder, project/device/API key management, search, chat, operator UI |
| **Agents** | Go, Gin, LangGraph, LangChain, inngestgo | Graph loading, workflow execution, step-level run tracking |
| **MCP** | Go, MCP go-sdk, replicate-go, GORM | OCR, descriptions, embeddings, segmentation, similarity search, retrieval tools |
| **Storage** | Postgres 18 + pgvector, S3-compatible, Redis | Documents, derived artifacts, vector search, sessions, realtime fanout |
| **Client** | Flutter, Dart, flutter_gemma, GenUI, fpdart | Optional demo client for capture, preview, and GenUI experiments |
