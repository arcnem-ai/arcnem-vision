---
title: APIの例
description: ワークフローキー経由の自動取り込み、ダッシュボード経由の単発アップロード、ワークフロー投入、リアルタイム監視の例。
---

Arcnem Vision には、大きく分けて2つの運用APIがあります。

- **ワークフローAPIキー経路**: 自動取り込み向け
- **ダッシュボード / セッション経路**: 運用担当者によるアップロード、閲覧、ワークフロー投入向け

エージェントは [OAuth MCP](/ja/guides/oauth-mcp/) から接続し、許可された範囲でワークフローの編集・実行やドキュメントの参照を行えます。

## ワークフローキー経路

この経路は、ワークフローキーのクライアントや外部連携から自動で画像を受け取るためのものです。
...
3番目の `ack` の後で、API はオブジェクトを検証し、ドキュメントを作成し、`document/process.upload` を発火します。以後は、そのワークフローキーに割り当てられたワークフローが読み込まれ、OCR、説明文生成、埋め込み、分岐、セグメンテーションなどが実行されます。

レスポンスには `processing.status`（`queued`、`skipped`（`workflow_unavailable`）、`failed`（`processing_enqueue_failed`））が含まれます。確定は繰り返しても安全です。同じ `objectKey` をもう一度送ると同じドキュメントが返り、キューへの登録が再試行されます。処理がキューに登録された後は、確定を繰り返してもイベントは再送されません。また、イベントIDはドキュメントごとに固定されています。再試行によってアップロードが二重に処理されることはありません。
